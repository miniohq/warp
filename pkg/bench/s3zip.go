...
func (g *S3Zip) Prepare(ctx context.Context) error {
	...
	_, err := client.GoClient.PutObject(ctx, g.Bucket, g.ZipObjName, pr, -1, g.PutOpts)
	pr.CloseWithError(err)
	if err == nil {
		var opts minio.GetObjectOptions
		opts.Set("x-minio-extract", "true")

		oi, err2 := client.GoClient.GetObject(ctx, g.Bucket, path.Join(g.ZipObjName, g.objects[0].Name), opts)
		if err2 != nil {
			err = err2
		}
		if err == nil {
	...
	for i := 0; i < g.Concurrency; i++ {
		go func(i int) {
			...
			for {
				...
				o, err := client.GoClient.GetObject(nonTerm, g.Bucket, op.File, opts)
				if err != nil {
